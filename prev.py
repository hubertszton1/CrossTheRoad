    ...
    def draw(self):
        ...
        if self.left:
            if self.up:
                ...
                self.hitbox = pygame.mask.from_surface(WALK_UP_LEFT[self.walk_count // 8])
            elif self.down:
                SCREEN.blit(WALK_DOWN_LEFT[self.walk_count // 8], (self.x, self.y))
                self.hitbox = pygame.mask.from_surface(WALK_DOWN_LEFT[self.walk_count // 8])
            else:
                SCREEN.blit(WALK_LEFT[self.walk_count // 8], (self.x, self.y))
                self.hitbox = pygame.mask.from_surface(WALK_LEFT[self.walk_count // 8])
            self.walk_count += 1
        elif self.right:
            if self.up:
                SCREEN.blit(WALK_UP_RIGHT[self.walk_count // 8], (self.x, self.y))
                self.hitbox = pygame.mask.from_surface(WALK_UP_RIGHT[self.walk_count // 8])
            elif self.down:
                SCREEN.blit(WALK_DOWN_RIGHT[self.walk_count // 8], (self.x, self.y))
                self.hitbox = pygame.mask.from_surface(WALK_DOWN_RIGHT[self.walk_count // 8])
            else:
                SCREEN.blit(WALK_RIGHT[self.walk_count // 8], (self.x, self.y))
                self.hitbox = pygame.mask.from_surface(WALK_RIGHT[self.walk_count // 8])
            self.walk_count += 1
        elif self.up:
            SCREEN.blit(WALK_UP[self.walk_count // 8], (self.x, self.y))
            self.hitbox = pygame.mask.from_surface(WALK_UP[self.walk_count // 8])
            self.walk_count += 1
        elif self.down:
            SCREEN.blit(WALK_DOWN[self.walk_count // 8], (self.x, self.y))
            self.hitbox = pygame.mask.from_surface(WALK_DOWN[self.walk_count // 8])
            self.walk_count += 1
        else:
            SCREEN.blit(IDLE[self.walk_count // 8], (self.x, self.y))
            self.hitbox = pygame.mask.from_surface(IDLE[self.walk_count // 8])
            self.walk_count += 1
...
def game():
    ...
    while running:
        ...
        for car, index in cars:
            ...
            #offset_rect = player.hitbox.to_surface().get_rect()
